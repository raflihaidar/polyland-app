...
const getPaymentDetail = async () => {
  try {
    ...
    if (res.status === 200) {
      ...
      if (
        paymentDetail.value &&
        paymentDetail.value.status !== PaymentStatus.PENDING
      ) {
        router.push(
          `/cari-berkas/pembayaran/${paymentDetail.value.order_id}/result`,
        );
        return;
      }
...
<template>
  ...
  <div v-else-if="paymentDetail">
    <section class="text-center">
      ...
      <UBadge
        :label="formatRupiah(paymentDetail.amount)"
        color="primary"
        class="mt-3"
        size="xl"
      />
      ...
      <div
        class="text-sm mt-3 flex items-center justify-center gap-x-1"
    ...
    <UCard class="mt-5">
      ...
      <div
        class="flex justify-center items-center cursor-pointer gap-x-3 text-primary"
        @click="copyToClipboard(paymentDetail.qr_url)"
      >
        <p class="font-medium">Salin link</p>
        <UIcon
          v-if="!isCopySuccess"
          name="tabler:copy"
          @click="copyToClipboard(paymentDetail.qr_url)"
        />
        <UIcon v-else name="tabler:copy-check-filled" />
      </div>
    </UCard>
    <section
      class="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white p-4 flex flex-col gap-y-3"
    >
      <UButton
        label="Download QR"
  ...
  <div v-else>
    <UEmpty
      ...
      title="Tagihan pembayaran tidak ditemukan"
